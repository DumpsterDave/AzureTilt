<?php
	...
	class TiltOMS {
		...
		public function Generate_Entry_With_Hardware($Temp, $SG, $Beer, $Color, $Comment='None') {
			$TimeStamp = gmdate('Y-m-d\TH:i:s.v\Z');
			$f = fopen('./pid/curr.data', 'r');
		        $HardwareData = explode(',', fgets($f));
		        fclose($f);

		        $f = fopen('./pid/set.temp', 'r');
		        $SetTemp = fgets($f);
		        fclose($f);

		        $f = fopen('./pid/set.delta', 'r');
		        $SetDelta = fgets($f);
		        fclose($f);

			$Entry = "[{\"Timepoint\": \"{$TimeStamp}\", \"TiltTemp\": {$Temp}, \"SG\": {$SG}, \"Beer\": \"{$Beer}\", \"Color\": \"{$Color}\", \"Comment\": \"{$Comment}\", \"Source\": \"TiltPHP\", \"Voltage\": {$HardwareData[0]}, \"MainAmps\": {$HardwareData[1]}, \"HotAmps\": {$HardwareData[2]}, \"ColdAmps\": {$HardwareData[3]}, \"HardwareTemp\": {$HardwareData[4]}, \"HotState\": {$HardwareData[5]}, \"ColdState\": {$HardwareData[6]}, \"SetTemp\": {$SetTemp}, \"SetDelta\": {$SetDelta}}]";
			return $Entry;
		}
